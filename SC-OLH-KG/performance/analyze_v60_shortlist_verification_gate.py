...
def _search_identity(control_rows, challenger_rows):
    ...
    for key in keys:
        ...
        trial_primary = trial.get("optimization_x_recommended")
        if trial_primary is None:
            frozen = list(dict(
                trial.get("terminal_verification") or {}
            ).get("frozen_shortlist") or [])
            if frozen:
                trial_primary = frozen[0].get("point")
        if (
