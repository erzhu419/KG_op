#!/usr/bin/env python3
"""Audit the paired V51/V58 guard-decomposition gate."""

from __future__ import annotations

import argparse
import importlib.util
import json
from pathlib import Path


HERE = Path(__file__).resolve().parent
SPEC = importlib.util.spec_from_file_location(
    "v56_analysis",
    HERE / "analyze_v56_independent_confirmation_gate.py",
)
V56 = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(V56)

CONTROL = "v51_control"
CHALLENGER = "v58_guard_decomposed_support"
KNOWN = (CONTROL, CHALLENGER)


def _variant(experiment):
    marker = f"/{str(experiment).strip('/')}/"
    return next((name for name in KNOWN if f"/{name}/" in marker), None)


def load_rows(root):
    rows = []
    errors = []
    for path in sorted(Path(root).rglob("result.json")):
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            variant = _variant(payload.get("experiment_variant", ""))
            if variant is None:
                continue
            for raw in payload["rows"]:
                row = dict(raw)
                row["gate_variant"] = variant
                row["result_path"] = str(path)
                rows.append(row)
        except (OSError, ValueError, KeyError, TypeError) as exc:
            errors.append({"path": str(path), "error": str(exc)})
    return rows, errors


def _trace_guard_contract(trace):
    guard = dict(trace.get("guard_decomposition") or {})
    support = dict(trace.get("guard_decomposition_support") or {})
    labels = list(trace.get("exact_kg_active_action_labels") or [])
    mode = str(guard.get("dominant_mode", ""))
    expected_prefix = {
        "epistemic": "guard_epistemic_",
        "aleatoric": "guard_aleatoric_",
        "interior": "guard_safe_interior_",
    }.get(mode)
    return bool(
        guard.get("status") == "ok"
        and expected_prefix is not None
        and not bool(guard.get("target_oracle_used", True))
        and support.get("status") == "ok"
        and support.get("dominant_mode") == mode
        and not bool(support.get("target_oracle_used", True))
        and sum(label == "v51_baseline_new" for label in labels) >= 4
        and any(str(label).startswith(expected_prefix) for label in labels)
    )


def _contract(row, variant):
    if variant == CONTROL:
        return V56._contract(row, CONTROL)
    contract = dict(row.get("decision_backend_contract") or {})
    confirmations = V56._confirmations(row)
    traces = list(row.get("online_action_trace") or [])
    dominance = dict(row.get("posterior_dominance") or {})
    return bool(
        str(row.get("implementation_contract_id"))
        == "v58_guard_decomposed_action_support"
        and str(row.get("theory_contract_id"))
        == "v58_guard_decomposed_policy_improvement_v1"
        and str(contract.get("evaluate_or_replicate_new_action_policy"))
        == "canonical_plus_posterior_guard_decomposition"
        and int(contract.get(
            "evaluate_or_replicate_baseline_new_action_count", -1)) == 4
        and str(contract.get("policy_improvement_guard_mode"))
        == "independent_confirmation"
        and str(contract.get("policy_improvement_score_transform"))
        == "bounded_current_gain"
        and str(contract.get("terminal_rule")) == "posterior_bayes_risk"
        and int(contract.get(
            "policy_improvement_confirmation_samples", -1)) == 4096
        and dominance.get("enabled") is False
        and bool(confirmations)
        and all(V56._confirmation_contract_valid(item)
                for item in confirmations)
        and bool(traces)
        and all(_trace_guard_contract(trace) for trace in traces)
        and not bool(row.get("online_action_trace_target_oracle_used", True))
    )


def _minimum_margin(row):
    audit = dict(row.get("certificate_outcome_audit") or {})
    return V56._finite(audit.get("minimum_posterior_margin"))


def _paired_certificate_progress(control_rows, challenger_rows):
    control = {V56._key(row): row for row in control_rows}
    challenger = {V56._key(row): row for row in challenger_rows}
    keys = sorted(set(control) & set(challenger))
    wins = losses = ties = 0
    deltas = []
    for key in keys:
        baseline = _minimum_margin(control[key])
        trial = _minimum_margin(challenger[key])
        if baseline is None or trial is None:
            ties += 1
            continue
        delta = float(trial - baseline)
        deltas.append(delta)
        if delta < -1e-12:
            wins += 1
        elif delta > 1e-12:
            losses += 1
        else:
            ties += 1
    return {
        "paired_count": len(keys),
        "margin_improvement_count": wins,
        "margin_worsening_count": losses,
        "margin_tie_count": ties,
        "median_minimum_margin_delta": V56._median(deltas),
        "majority_nonworse": bool(keys and wins + ties >= losses),
    }


def _summary(rows):
    summary = V56._summary(rows)
    traces = [
        trace
        for row in rows
        for trace in list(row.get("online_action_trace") or [])
    ]
    modes = ("epistemic", "aleatoric", "interior")
    summary.update({
        "median_minimum_posterior_margin": V56._median(
            _minimum_margin(row) for row in rows),
        "guard_trace_count": len(traces),
        "guard_contract_valid_count": sum(
            _trace_guard_contract(trace) for trace in traces),
        "guard_mode_counts": {
            mode: sum(
                str(dict(trace.get("guard_decomposition") or {}).get(
                    "dominant_mode")) == mode
                for trace in traces
            )
            for mode in modes
        },
        "guard_anchor_replication_action_count": sum(
            "guard_aleatoric_anchor_replicate"
            in list(trace.get("exact_kg_active_action_labels") or [])
            for trace in traces
        ),
    })
    return summary


def analyze(root):
    rows, errors = load_rows(root)
    grouped = {
        variant: {
            domain: [
                row for row in rows
                if row["gate_variant"] == variant
                and str(row.get("heldout")) == domain
            ]
            for domain in V56.DOMAINS
        }
        for variant in KNOWN
    }
    flat = {
        variant: [
            row
            for domain_rows in grouped[variant].values()
            for row in domain_rows
        ]
        for variant in KNOWN
    }
    summaries = {
        variant: {
            domain: _summary(domain_rows)
            for domain, domain_rows in domains.items()
        }
        for variant, domains in grouped.items()
    }
    complete = all(
        len(grouped[variant][domain]) == 5
        for variant in KNOWN for domain in V56.DOMAINS
    )
    keys = {
        variant: {V56._key(row) for row in flat[variant]}
        for variant in KNOWN
    }
    paired = keys[CONTROL] == keys[CHALLENGER]
    contracts = {
        variant: all(_contract(row, variant) for row in flat[variant])
        for variant in KNOWN
    }
    performance = V56._paired_performance(
        flat[CONTROL], flat[CHALLENGER])
    certificate_progress = _paired_certificate_progress(
        flat[CONTROL], flat[CHALLENGER])
    challenger_audits = [
        dict(row.get("certificate_outcome_audit") or {})
        for row in flat[CHALLENGER]
    ]
    no_false_certificates = all(
        int(item.get("false_certificate_count", 0) or 0) == 0
        for item in challenger_audits
    )
    chance_certificate_nonvacuous = any(
        int(item.get("posterior_certified_count", 0) or 0) > 0
        for item in challenger_audits
    )
    confirmation_nonvacuous = any(
        bool(item.get("passed", False))
        for row in flat[CHALLENGER]
        for item in V56._confirmations(row)
    )
    formal_gate = bool(
        complete
        and not errors
        and paired
        and all(contracts.values())
        and no_false_certificates
    )
    promotion_gate = bool(
        formal_gate
        and confirmation_nonvacuous
        and chance_certificate_nonvacuous
        and performance["performance_noninferior"]
        and performance["feasibility_loss_count"] == 0
        and certificate_progress["majority_nonworse"]
    )
    return {
        "scope": "v58_guard_decomposition_gate",
        "root": str(Path(root)),
        "row_count": len(rows),
        "load_errors": errors,
        "complete_5_seed_matrix": complete,
        "paired_keys_match_control": paired,
        "contract_valid": contracts,
        "confirmation_nonvacuous": confirmation_nonvacuous,
        "chance_certificate_nonvacuous": chance_certificate_nonvacuous,
        "no_false_certificates": no_false_certificates,
        "paired_performance": performance,
        "paired_certificate_progress": certificate_progress,
        "formal_gate_passed": formal_gate,
        "promotion_gate_passed": promotion_gate,
        "gate_passed": promotion_gate,
        "summaries": summaries,
    }


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("root", type=Path)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    report = analyze(args.root)
    rendered = json.dumps(report, indent=2, sort_keys=True)
    if args.output is not None:
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(rendered + "\n", encoding="utf-8")
    print(rendered)
    raise SystemExit(0 if report["formal_gate_passed"] else 1)


if __name__ == "__main__":
    main()
