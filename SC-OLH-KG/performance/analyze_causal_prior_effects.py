...
def _paired_metrics(challenger, reference):
    """Metrics shared by prior-profile and proposal-mode contrasts."""
    initial_challenger_regret = _finite(
        challenger.get("initial_best_feasible_regret"))
    initial_reference_regret = _finite(
        reference.get("initial_best_feasible_regret"))
    final_challenger_regret = _finite(challenger.get("feasible_regret"))
    final_reference_regret = _finite(reference.get("feasible_regret"))
    return {
        ...
        "initial_regret_delta": (
            initial_challenger_regret - initial_reference_regret
            if (
                challenger.get("initial_has_true_feasible") is True
                and reference.get("initial_has_true_feasible") is True
                and initial_challenger_regret is not None
                and initial_reference_regret is not None
            )
            else None
        ),
        "final_regret_delta": (
            final_challenger_regret - final_reference_regret
            if (
                challenger.get("true_feasible") is True
                and reference.get("true_feasible") is True
                and final_challenger_regret is not None
                and final_reference_regret is not None
            )
            else None
        ),
    }
...
def build_proposal_mode_effects(
        rows,
        challenger_mode="risk_coordinate_atlas",
        reference_mode="rank_spanning"):
    """Pair dimension-equivariant and raw-profile proposals seed by seed."""
    index = {}
    for row in rows:
        variant = parse_causal_variant(row.get("variant"))
        if variant is None or row.get("status") != "ok":
        ...
        if key in index:
            raise ValueError(f"duplicate proposal-mode cell {key}")
        index[key] = row

    base_keys = sorted({key[:-1] for key in index})
    pairs = []
    for base in base_keys:
        challenger = index.get((*base, challenger_mode))
        reference = index.get((*base, reference_mode))
        if challenger is None or reference is None:
            continue
        pairs.append({
            "run_id": base[0],
            ...
            "n0": base[7],
            "source_calls": base[8],
            "contrast": "proposal_mode",
            "challenger_proposal_mode": challenger_mode,
            "reference_proposal_mode": reference_mode,
            **_paired_metrics(challenger, reference),
        })
    return pairs
...
def summarize_proposal_mode_pairs(pairs):
    groups = {}
    fields = (
        "run_id",
        "causal_mode",
        ...
        "source_calls",
        "contrast",
        "challenger_proposal_mode",
        "reference_proposal_mode",
    )
    for pair in pairs:
        key = tuple(pair[field] for field in fields)
        groups.setdefault(key, []).append(pair)
...
def main():
    ...
    _write_csv(args.out_dir / "proposal_mode_paired_rows.csv", proposal_pairs)
    _write_csv(
        args.out_dir / "proposal_mode_paired_effects.csv", proposal_summaries)
    audit = {
        "schema_version": 1,
        ...
        "proposal_mode_paired_row_count": len(proposal_pairs),
        "proposal_mode_paired_effect_count": len(proposal_summaries),
        "safety_contract": {
            "accepted_filename": "result.json",
            "checkpoint_or_pickle_read": False,
        },
        "effects": summaries,
        "proposal_mode_effects": proposal_summaries,
    }
    ...
    print(json.dumps({
        ...
        "proposal_mode_paired_rows": len(proposal_pairs),
        "proposal_mode_paired_effects": len(proposal_summaries),
