"""Strict source-only gate for continuous boundary-family synthesis."""
...
def _fit_synthesis_model_from_rows(args, rows):
...
def run_gate(args, heldout, target_seed):
    result = shared.run_gate(
        ...
        heldout,
        target_seed,
        model_builder=_fit_synthesis_model_from_rows,
    )
...
def build_parser():
    ...
    parser.description = (
        "Strict source-only TCB-V4 boundary-family synthesis gate")
    parser.add_argument("--coefficient-ridge", type=float, default=0.1)
    parser.add_argument(
        "--coefficient-prior-strength", type=float, default=1.0)
...
def main():
    ...
    print(
        f"DONE tcb_v4_synthesis_gate rows={len(result['rows'])}",
        flush=True,
    )
