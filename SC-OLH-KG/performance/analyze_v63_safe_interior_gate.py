#!/usr/bin/env python3
"""Audit V63 cumulative-risk safe-interior verification against V51."""
...
V62 = _load(
    "v62_analysis_v63",
    "analyze_v62_exact_tolerance_gate.py",
)
...
def _verification_contract(row):
    ...
    shortlist = list(verification.get("frozen_shortlist") or [])
    ...
    attempts_valid = bool(
        ...
        and all(
            V62._attempt_contract(attempt, index, expected_budgets[index])
    ...
    shortlist_valid = bool(
        ...
        and shortlist[0].get("shortlist_role")
        == "posterior_bayes_primary"
        and shortlist[1].get("shortlist_role")
        == "posterior_safe_interior_diversified"
        and shortlist[1].get("selection_contract") == SELECTION_CONTRACT
        and shortlist[1].get("candidate_universe")
        == "frozen_initial_atlas"
        and int(shortlist[1].get("candidate_universe_size", -1)) == 10
        and int(shortlist[1].get("eligible_count", 0)) >= 1
        and float(shortlist[1].get("probability_slack", -1.0)) == 0.05
        and str(shortlist[1].get("coordinate_source"))
        == "cumulative_risk_psi=(A,N)"
        and int(shortlist[1].get("coordinate_dimension", 0)) > 0
        and shortlist[1].get("target_labels_used") is False
        and shortlist[1].get("target_oracle_used") is False
        and shortlist[1].get("verification_samples_used") is False
    )
    return bool(
        ...
        and str(verification.get("shortlist_mode")) == SHORTLIST_MODE
