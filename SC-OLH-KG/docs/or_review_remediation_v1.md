...
| Strong target schema | Run declared-schema, schema-blind, coordinate-permutation, descriptor-conditioned, and domain-blind controls. | Generator and controls implemented. Evaluation patch V3 fixes declared-schema plus domain-blind as the primary family; descriptor-conditioned results are supplementary. Confirmatory schema matrix pending. |
| Hyperparameter sensitivity | Register alpha, safe mass, n0, effective rank, source task/profile/replication budgets, retained frequency, frequency penalty, rank metric weights, and first-center tradeoff. | Restartable OFAT matrix implemented. Evaluation patch V3 records the active-rank override and treats the registered `n0=N` budget change as one intervention. Confirmatory execution pending. |
