...
| Candidate verifier validity and power | Use a frozen shortlist and an exact all-success binomial candidate test with explicit familywise spending. Generate exact power/nonvacuity curves over true safety depth and budget. | Candidate validity is machine checked; power runner implemented; confirmatory power table pending. The rule is conservative and is not claimed sample optimal. |
| Energy temporal independence | Draw start indices independently with replacement from one fixed verification-year empirical set. Physical windows may overlap. The certificate is only for that empirical distribution; no iid future-calendar claim is allowed. | Enforced in code and information contract. |
| Equal-total-cost wording | Report source, target search, actual verification, maximum verification, actual all-in, maximum all-in, and amortized all-in calls. Add a fixed-cap comparison of 384 source + 10 target versus 394 target-only calls. | Implemented; confirmatory all-in frontier pending. |
| Transfer baselines receive our atlas | Rename existing shared-atlas results conceptually as a backend comparison. Audit native end-to-end transfer pipelines under identical source information, target budget, and verifier. | Native-pipeline audit remains pending and blocks final manuscript claims. |
| Missing structured controls | Add generic DCT maximin, random low-frequency, natural blockwise or constant grids, raw Sobol, and a labeled finite-library oracle upper bound. | Implemented in stress and energy V2. The oracle arm is excluded from method comparisons. |
...
| Pseudoreplication | Treat randomized target tasks, markets, and geographic regions separately from repeated algorithm seeds. Prohibit pooled domain-seed population claims. | Analysis contracts implemented. |
| Strong target schema | Run declared-schema, schema-blind, coordinate-permutation, descriptor-conditioned, and domain-blind controls. | Generator and controls implemented; confirmatory schema matrix pending. |
| Hyperparameter sensitivity | Register alpha, safe mass, n0, effective rank, source task/profile/replication budgets, retained frequency, frequency penalty, rank metric weights, and first-center tradeoff. | Restartable OFAT matrix implemented; confirmatory execution pending. |
