"""Qualify operational probes for one verified, materialized shadow child."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all seven input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all seven input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Qualify a verified materialized shadow child with fresh local "
            "holdout and stress evidence under a new evaluator epoch."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        result = qualify_shadow_child_operational_probes(
            competition_input,
            ...
            input_artifacts=qualification_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("qualification core must return a JSON object")
        ...
        print(f"invalid shadow-child-probe-qualification input: {exc}", file=sys.stderr)
        return 2
    return 0
