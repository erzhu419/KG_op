"""Freshly qualify and route one restricted shadow theory state."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all eighteen input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all eighteen input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Freshly requalify a restricted shadow theory against its exact "
            "source rollback target, then emit one bounded cycle route."
        )
    )
    ...
    parser.add_argument(
        "--out", type=Path, help="atomically copy the canonical adjudication report"
    )
    return parser
    ...
    try:
        ...
        result = qualify_adjudicate_and_route_shadow_post_restriction(
            *(loaded[name] for name, _ in labels),
            ...
            input_artifacts=adjudication_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("post-restriction-adjudication core must return a JSON object")
        ...
        print(f"invalid shadow-post-restriction-adjudication input: {exc}", file=sys.stderr)
        return 2
    return 0
