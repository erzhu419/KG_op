"""Compete and materialize one bounded shadow robust interval restriction."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all fifteen input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all fifteen input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Compete the fixed robust interval contraction registry on fresh, "
            "caller-supplied static rows and materialize at most one bounded "
            "shadow restriction."
        )
    )
    ...
    parser.add_argument("--expected-probe-report-digest", required=True)
    parser.add_argument("--expected-restriction-input-digest", required=True)
    parser.add_argument("--expected-restriction-contract-digest", required=True)
    parser.add_argument(
        "--out", type=Path, help="atomically copy the canonical restriction report"
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        result = compete_and_materialize_shadow_robust_interval_restriction(
            loaded["competition_input"],
            loaded["competition_contract"],
            loaded["competition_report"],
            loaded["transition_contract"],
            loaded["transition_report"],
            loaded["qualification_input"],
            loaded["qualification_contract"],
            loaded["qualification_report"],
            loaded["review_contract"],
            loaded["review_report"],
            loaded["probe_input"],
            loaded["probe_contract"],
            loaded["probe_report"],
            loaded["restriction_input"],
            loaded["restriction_contract"],
            ...
            input_artifacts=restriction_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("robust-interval-restriction core must return a JSON object")
        ...
        print(f"invalid shadow-robust-interval-restriction input: {exc}", file=sys.stderr)
        return 2
    return 0
