"""Emit one bounded interval/multi-Q recompetition seed by exact replay."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all twenty-one input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all twenty-one input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Exact-replay one post-restriction adjudication and emit only its "
            "bounded interval/multi-Q recompetition seed."
        )
    )
    ...
    parser.add_argument(
        "--out", type=Path, help="atomically copy the canonical adapter report"
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if not isinstance(report, dict):
            raise ValueError("interval/multi-Q adapter core must return a JSON object")
        ...
        print(f"invalid shadow-interval-multi-q-adapter input: {exc}", file=sys.stderr)
        return 2
    return 0
