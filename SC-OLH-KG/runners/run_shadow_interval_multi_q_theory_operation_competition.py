"""Run one bounded interval/multi-Q shadow competition by exact replay."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        # Inputs are already required to be absolute non-symlink regular files.
        # Normalize lexical aliases without repeating realpath resolution for
        # every one of the 276 pairwise KATs; inode identity still catches hard
        # links and aliases through symlinked parent directories.
        resolved = Path(os.path.abspath(os.fspath(path)))
        if resolved in resolved_paths:
            raise ValueError("all twenty-four input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all twenty-four input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Exact-replay the bounded shadow chain and run one validation-only "
            "finite-interval/two-Q theory-operation competition."
        )
    )
    ...
    parser.add_argument(
        "--out", type=Path, help="atomically copy the canonical competition report"
    )
    return parser
    ...
    try:
        ...
        result = run_shadow_interval_multi_q_theory_operation_competition(
            *(loaded[name] for name, _ in labels),
            ...
            input_artifacts=interval_competition_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("interval/multi-Q competition core must return a JSON object")
        ...
        print(f"invalid shadow-interval-multi-q-competition input: {exc}", file=sys.stderr)
        return 2
    return 0
