"""Materialize one verified interval/multi-Q selection as a shadow transition."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all twenty-six input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all twenty-six input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Exact-replay the bounded shadow chain and materialize a verified "
            "interval/multi-Q selected candidate as an unqualified shadow child."
        )
    )
    ...
    try:
        ...
        result = materialize_shadow_interval_multi_q_theory_transition(
            *(loaded[name] for name, _ in labels),
            ...
            input_artifacts=interval_transition_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("interval/multi-Q transition core must return a JSON object")
        ...
        print(f"invalid shadow-interval-multi-q-transition input: {exc}", file=sys.stderr)
        return 2
    return 0
