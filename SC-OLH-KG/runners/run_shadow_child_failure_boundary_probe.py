"""Compile and evaluate one local shadow failure-boundary probe."""
...
def _require_distinct_inputs(inputs: tuple[Path, ...]) -> None:
    ...
    for path in inputs:
        ...
        if resolved in resolved_paths:
            raise ValueError("all twelve input paths must resolve distinctly")
        resolved_paths.add(resolved)
        stat = path.stat()
        inode = (stat.st_dev, stat.st_ino)
        if inode in inodes:
            raise ValueError("all twelve input files must have distinct inodes")
        inodes.add(inode)
...
def _parser() -> argparse.ArgumentParser:
    parser = _StrictArgumentParser(
        description=(
            "Compile and evaluate one transition-specific failure-boundary "
            "probe on caller-supplied static rows for an exact-replayed "
            "review-ready shadow child."
        )
    )
    ...
    parser.add_argument(
        "--probe-contract", type=Path, default=DEFAULT_PROBE_CONTRACT
    )
    ...
    parser.add_argument("--expected-review-report-digest", required=True)
    parser.add_argument("--expected-probe-input-digest", required=True)
    parser.add_argument("--expected-probe-contract-digest", required=True)
    parser.add_argument(
        "--out", type=Path, help="atomically copy the canonical probe report"
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        review_report_path = _require_input_file(
            args.review_report, "--review-report"
        )
        probe_input_path = _require_input_file(args.probe_input, "--probe-input")
        probe_contract_path = _require_input_file(
            args.probe_contract, "--probe-contract"
        )
        inputs = (
        ...
        review_report, review_report_artifact = _load_json(
            review_report_path, "--review-report"
        )
        probe_input, probe_input_artifact = _load_json(
            probe_input_path, "--probe-input"
        )
        probe_contract, probe_contract_artifact = _load_json(
            probe_contract_path, "--probe-contract"
        )
        ...
        probe_input_artifacts = {
            ...
            "competition_input_json": competition_input_artifact,
            "competition_report_json": competition_report_artifact,
            "probe_contract_json": probe_contract_artifact,
            "probe_input_json": probe_input_artifact,
            "qualification_contract_json": qualification_contract_artifact,
            "qualification_input_json": qualification_input_artifact,
            "qualification_report_json": qualification_report_artifact,
            "review_contract_json": review_contract_artifact,
            "review_report_json": review_report_artifact,
            "transition_contract_json": transition_contract_artifact,
            "transition_report_json": transition_report_artifact,
        }
        result = expand_and_evaluate_shadow_child_failure_boundary_probe(
            ...
            input_artifacts=probe_input_artifacts,
        )
        report = result.to_dict() if hasattr(result, "to_dict") else result
        if not isinstance(report, dict):
            raise ValueError("failure-boundary-probe core must return a JSON object")
        ...
        print(f"invalid shadow-child-failure-boundary-probe input: {exc}", file=sys.stderr)
        return 2
    return 0
