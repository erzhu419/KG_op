...
SUBMIT_PATH = (
    REPO / "scripts/submit_scolhkg_mean_alignment_v9_offline_gate_scheduler.py")
SUBMIT_SPEC = importlib.util.spec_from_file_location(
    "mean_v9_submit", SUBMIT_PATH)
submit = importlib.util.module_from_spec(SUBMIT_SPEC)
SUBMIT_SPEC.loader.exec_module(submit)

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))
from performance.analyze_mean_alignment_v9_gate import (  # noqa: E402
    CHALLENGERS,
    MISSPECIFICATION,
    SCENARIOS,
    VARIANTS,
...
def _args(tmp_path):
    ...
    return type("Args", (), {
        ...
        "source_run_id": submit.base.base.DEFAULT_SOURCE_RUN_ID,
        "run_id": "mean-v9",
        "rank": 4,
        "source_d": 50,
        "d": 1000,
        ...
        "contrast_scale": 1.0,
        "python": submit.base.base.REMOTE_PYTHON,
        "cpu": 1,
        "ram_mb": 4096,
    })()


def test_v9_submitter_builds_hierarchical_residual_matrix(tmp_path):
    specs = submit.build_specs(_args(tmp_path))
    assert len(specs) == 5 * 4 * 5
    assert len({spec["signature"] for spec in specs}) == len(specs)
    mixture = next(spec for spec in specs
                   if "/v8_mixture_control/" in spec["signature"])
    assert (
        "--source-constraint-mean-adaptation-mode "
        "sequential_evidence_mixture" in mixture["cmd"])
    residual = next(spec for spec in specs
                    if "/hierarchical_residual/" in spec["signature"])
    assert "--source-constraint-mean-adaptation-mode frozen" in residual["cmd"]
    assert "--hvd-source-task-weight-mode independent" in residual["cmd"]
    directional = next(
        spec for spec in specs
        if "/hierarchical_residual_directional/" in spec["signature"])
    assert (
        "--source-constraint-mean-misspecification-mode "
        "predictive_scale_directional" in directional["cmd"])
    assert all("--hvd-source-task-weight-mode independent" in spec["cmd"]
               for spec in specs)
    assert all(spec["allowed_nodes"] == list(submit.CPU_NODES)
               for spec in specs)


def test_v9_analyzer_accepts_source_residual_posterior(tmp_path):
    rows = []
    for variant in VARIANTS:
        ...
        for domain, shock in SCENARIOS:
            ...
            for seed in range(5):
                ...
                prior = {
                    "adaptation_mode": (
                        "frozen_aggregate" if challenger
