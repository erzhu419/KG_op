...
SUBMIT_PATH = (
    REPO / "scripts/submit_scolhkg_mean_alignment_v30_student_t_gate_scheduler.py")
SUBMIT_SPEC = importlib.util.spec_from_file_location(
    "mean_v30_submit", SUBMIT_PATH)
...
def test_v30_submitter_pairs_posterior_only_student_t_calibrations(tmp_path):
    ...
    for variant, profile in submit.VARIANTS.items():
        ...
        assert all(
            ...
            for spec in selected)
        assert all(
            "--source-constraint-mean-misspecification-delta 0.05"
            in spec["cmd"] for spec in selected)
