...
SUBMIT_PATH = (
    REPO
    / "scripts/submit_scolhkg_mean_alignment_v35b_frontier_isolation_gate_scheduler.py"
)
SUBMIT_SPEC = importlib.util.spec_from_file_location(
    "mean_v35b_submit", SUBMIT_PATH)
...
def test_v35b_isolates_confidence_authority_from_frontier_exploration(tmp_path):
    specs = submit.build_specs(_args(tmp_path))
    assert len(specs) == 2 * 3 * 5
    assert all(
        "--terminal-frontier-candidate-count 1" in spec["cmd"]
        for spec in specs
    )
