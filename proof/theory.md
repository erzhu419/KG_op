...
2. The internal posterior theory certificate was vacuous at the earlier V51
   budget: both 60-run matrices had empty certified sets, including zero of
   129 truly feasible evaluated points in the pre-repair control. V64 does not
   relabel that certificate. It adds a distinct frozen independent deployment
   certificate, which certified 60/60 fresh policies with zero false
   certificates under precommitted 80/96 replication budgets. Paper tables
   report model-certificate coverage and terminal-deployment coverage
   separately.
...
| V63 cumulative-risk safe-interior verification | `SCOLHKG/Real/GaussianReplicationCertificate.lean`, `SCOLHKG/Measure/GaussianReplicationCertificate.lean` | V63 freezes the V51 Bayes-risk primary and one ambiguity-diversified initial-atlas support from a posterior violation sublevel set in `psi=(A,N)`, before seeing any verification label. Lean records the initial-atlas/sublevel selector contract, proves shortlist immutability under verification, and proves family-wise false deployment at most `delta_1+delta_2` for the selected pair. The claim is selector-agnostic safety, not a theorem that the maximin support must certify; coverage improvement remains a fresh empirical gate. |
| V64 powered safe-interior verification | `SCOLHKG/Real/GaussianReplicationCertificate.lean`, `SCOLHKG/Measure/GaussianReplicationCertificate.lean` | V64 changes only the precommitted rank-1 verification budget from 64 to 80 while retaining the 96-call frozen support and `0.025+0.025` familywise split. Lean proves the exact 80/96 budget cap by specialization of the asymmetric-budget theorem. Since the selector is frozen before independent verification and error spending is unchanged, V63's familywise false-deployment bound applies verbatim; higher certification power is an empirical claim tested on untouched seeds. |
| Controlled-heteroscedastic V9 verification-aware terminal policy | `SCOLHKG/Real/GaussianReplicationCertificate.lean`, `SCOLHKG/Measure/GaussianReplicationCertificate.lean` | V9 freezes three observed policies before independent labels: the best posterior objective among posterior-median feasible candidates, the strict posterior-feasible primary, and the cumulative-risk safe-interior support. Lean proves the selector contract is verification-invariant, the asymmetric cost is at most `search + 80 + 128 + 128`, and candidate-wise false-certificate bounds with `delta_1 + delta_2 + delta_3 <= delta` imply family-wise false deployment at most `delta`, without assuming independent certificate events. Objective quality and certification power are empirical claims; `P(violation) <= 0.5` is only challenger admission and never a safety certificate. |
| Method-independent paper verification | `SCOLHKG/Real/MethodIndependentTerminalVerification.lean` | The optimizer identity is explicit but inert after shortlist freeze. For every finite collection of SC, transfer, or online-SOTA methods, candidate-wise false-certificate bounds and the common error split imply the same family-wise false-deployment bound method by method. The file also proves exact `source + search + verification` cost decomposition and the powered 80/96 total-cost cap. |
