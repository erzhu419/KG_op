...
| Frozen source-consensus proposal and committed safety shortlist | `LearnedMetaPrior._fit_source_consensus_templates` aggregates within-source empirical chance-margin and objective ranks over one shared universal archive; `risk_objective_initial_candidates` selects source safety-objective Pareto elites before dimension-free maximin filling; `initial_universal_candidates` retains the legacy rank-spanning design; `observed_safety_reserved` restricts its protected shortlist to charged members of that design, preserves a safety alias when the first arm duplicates minimum Bayes risk, and `commit_before_switch` finishes all reserved arms before changing to posterior-only experts | `SCOLHKG.Real.SourceConsensusCommit.strictlyMonotone_order_invariant` proves domainwise rank invariance under every strictly increasing rescaling; the safety-objective scalarization lemmas prove nonnegativity and weak/strict Pareto monotonicity; `source_frozen_proposal_target_noninterference` records target-name noninterference; the remaining ordering and commitment theorems prove shortlist completion within the reserved suffix budget |
