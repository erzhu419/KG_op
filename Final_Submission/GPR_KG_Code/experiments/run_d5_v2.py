"""
d=5 v2 Experiments: RZDT1, RZDT2, and legacy RZDT5 with original paper settings.

Key differences from d5_hetero:
  - RZDT1/RZDT2: L=100, x_j in {0,...,100}
  - RZDT5: x1 in {0,...,30}, x_j in {0,...,5} for j>=2
  - tau=0 fixed (not calibrated)
  - GPR-KG uses data-driven lambda_i/prior_var, K1=50, K2=2,
    w_vepm=1, n_thr=20, and inner NSGA-II pop_size=100, n_gen=50
  - heteroscedastic=True (sigma_level=0.04)

Results saved to results/d5_v2/

Usage:
    cd GPR_KG_Code
    python -m experiments.run_d5_v2
    python -m experiments.run_d5_v2 --method GPR-KG --problem RZDT1
    python -m experiments.run_d5_v2 --n_reps 10
"""
...
from gpr_kg import RZDT1, RZDT2, RZDT5, compute_hypervolume_2d
from metrics import compute_igd, compute_cvr
from experiments.config import (
    DEFAULT_N, DEFAULT_N0, DEFAULT_D,
    DEFAULT_ALPHA, N_REPS, SEED_BASE, REF_POINT, HV_EVAL_INTERVAL,
)
...
# ── output root ──────────────────────────────────────────────
RESULTS_ROOT = os.path.join(BASE_DIR, "results", "d5_v2")
...
# ── problem registry (v2 settings) ───────────────────────────
V2_SIGMA = 0.04  # baseline noise for RZDT1/2/legacy RZDT5 (heteroscedastic)

def make_problem(problem_name, d=DEFAULT_D, sigma=V2_SIGMA, alpha=DEFAULT_ALPHA):
    """Instantiate problem with v2 settings and tau=0."""
    if problem_name == "RZDT1":
        prob = RZDT1(d=d, L=100, sigma=sigma, heteroscedastic=True, alpha=alpha)
    elif problem_name == "RZDT2":
        prob = RZDT2(d=d, L=100, sigma=sigma, heteroscedastic=True, alpha=alpha)
    elif problem_name == "RZDT5":
        prob = RZDT5(d=d, L=30, sigma=sigma, heteroscedastic=True, alpha=alpha)
    else:
...
ALL_PROBLEM_NAMES = ["RZDT1", "RZDT2", "RZDT5"]
...
def save_problem_meta(problem_name):
    """Save problem metadata (tau, true PF, etc.) to results/d5_v2/problems/."""
...
def run_d5_v2(methods=None, problems=None, n_reps=N_REPS):
    if methods is None:
    ...
    print("\n" + "=" * 70)
    print("  d=5 v2 Experiments (original paper settings)")
    print(f"  Problems : {problems}")
    print(f"  Methods  : {methods}")
    print(f"  Reps     : {n_reps}  |  N={DEFAULT_N}  n0={DEFAULT_N0}")
    print(f"  tau=0 fixed, heteroscedastic=True, sigma=0.04, K2=2, w_vepm=1")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run d=5 v2 experiments (original paper settings)")
    ...
    run_d5_v2(methods=methods, problems=problems, n_reps=args.n_reps)
