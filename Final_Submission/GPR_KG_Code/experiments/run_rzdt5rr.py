"""
RZDT5_RR Experiment: RZDT5_R geometry with the strict original constraint tau=0.
...
Problem settings:
  ...
  - Chance constraint: P(f3 <= tau) >= 0.95,  tau = 0.0, alpha = 0.05
  - Heteroscedastic = True
  - Expected TPOS: 46 (x_1 in {0,...,45}, x_j=0 for j>=2)
  - TPOS variance ratio: ~5.5x

Results saved to results/rzdt5rr/RZDT5_RR/ (same format as results/rzdt5r/).

Usage:
    cd GPR_KG_Code
    python -m experiments.run_rzdt5rr
    python -m experiments.run_rzdt5rr --method GPR-KG
    python -m experiments.run_rzdt5rr --n_reps 10
"""
...
from gpr_kg import RZDT5_RR, compute_hypervolume_2d
...
# ── output root ──────────────────────────────────────────────
RESULTS_ROOT = os.path.join(BASE_DIR, "results", "rzdt5rr")
...
# ── problem settings (RZDT5_RR) ──────────────────────────────
RZDT5RR_SIGMA = 0.04
RZDT5RR_TAU   = 0.0

def make_problem(problem_name="RZDT5_RR", d=DEFAULT_D,
                 sigma=RZDT5RR_SIGMA, alpha=DEFAULT_ALPHA):
    """Instantiate RZDT5_RR with tau=0.0."""
    if problem_name != "RZDT5_RR":
        raise ValueError(f"Unknown problem: {problem_name}")
    prob = RZDT5_RR(d=d, sigma=sigma, heteroscedastic=True, alpha=alpha)
    prob.tau = RZDT5RR_TAU
    return prob

ALL_PROBLEM_NAMES = ["RZDT5_RR"]
...
def save_problem_meta(problem_name):
    """Save problem metadata (tau, true PF, etc.) to results/rzdt5rr/problems/."""
...
def run_rzdt5rr(methods=None, problems=None, n_reps=N_REPS):
    if methods is None:
    ...
    print("\n" + "=" * 70)
    print("  RZDT5_RR Experiments (RZDT5_R geometry, strict tau=0)")
    print(f"  Problems : {problems}")
    print(f"  Methods  : {methods}")
    print(f"  Reps     : {n_reps}  |  N={DEFAULT_N}  n0={DEFAULT_N0}")
    print(f"  tau={RZDT5RR_TAU}, heteroscedastic=True, sigma={RZDT5RR_SIGMA}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run RZDT5_RR experiment (RZDT5_R geometry, strict tau=0)")
    ...
    run_rzdt5rr(methods=methods, problems=problems, n_reps=args.n_reps)
