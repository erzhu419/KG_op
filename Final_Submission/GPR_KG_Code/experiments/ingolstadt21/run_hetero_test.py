"""
Phase 1: Heteroscedastic noise verification on ingolstadt21.

Mirrors experiments.intas.run_hetero_test but uses Ingolstadt21Problem.

Env overrides:
  INGOLSTADT21_HETERO_M    number of plans  (default 20)
  INGOLSTADT21_HETERO_R    reps per plan    (default 10)

Usage:
    python -u -m experiments.ingolstadt21.run_hetero_test
"""
...
DEFAULT_M = int(os.environ.get("INGOLSTADT21_HETERO_M", 20))
DEFAULT_R = int(os.environ.get("INGOLSTADT21_HETERO_R", 10))


def _lhs_int_plans(prob: Ingolstadt21Problem, m: int, seed: int = 0):
...
def run(m: int = None, r: int = None, seed: int = 42):
    ...
    if os.path.exists(out_path):
        print("Final hetero_test.json already exists. Delete to recompute.")
    ...
    if os.path.exists(plans_path):
        ...
        if m_cached == m:
            ...
            extra = _lhs_int_plans(prob, m - m_cached, seed=seed + m_cached * 101)
            plans = plans + extra
            with open(plans_path, 'w') as f:
                json.dump({'plans': [list(p) for p in plans], 'm': m, 'seed': seed,
                           'extended_from': m_cached}, f, indent=2)
            print(f"Extended cached plans m={m_cached}->{m}", flush=True)
        else:
            raise AssertionError(
                f"Plans cached with m={m_cached} but requested m={m}; "
                f"delete {plans_path} to regenerate.")
    else:
    ...
    total_needed = m * r
    print(f"\nPhase 1: ingolstadt21 hetero noise verification "
          f"(M={m} plans x R={r} reps)", flush=True)
    print(f"  d={prob.d}  T0={prob.T0:.1f}s  A0={prob.A0:.4f}  E0={prob.E0:.1f}kg",
          flush=True)
    if total_done > 0:
        print(f"  Resuming from partial: {total_done}/{total_needed} done.",
              flush=True)
    ...
    for i, plan in enumerate(plans):
        ...
        for rep in range(done_for_plan, r):
            ...
            record = {'plan_id': pid, 'rep': rep, 'y': y_list,
                      'sim_sec': float(sim_dt), 'ts': time.time()}
            ...
            print(f"  plan {pid+1:02d}/{m} rep {rep+1:02d}/{r}  "
                  f"f1={y[0]:.3f}  f2={y[1]:.3f}  f3={y[2]:.3f}  "
                  f"dt={sim_dt:.0f}s  total={total_done}/{total_needed}  "
    ...
    for i, plan in enumerate(plans):
        ...
        results.append({
            'plan_id':  i, 'plan': list(plan),
            'mean_f1':  float(mean[0]), 'mean_f2':  float(mean[1]), 'mean_f3':  float(mean[2]),
            'var_f1':   float(var[0]),  'var_f2':   float(var[1]),  'var_f3':   float(var[2]),
            'reps':     arr.tolist(),
        })
    ...
    for j, name in enumerate(['f1', 'f2', 'f3']):
        ...
        hetero_by_obj[name] = {
            'levene_stat':   float(lev_s),  'levene_p':   float(lev_p),
            'bartlett_stat': float(bar_s),  'bartlett_p': float(bar_p),
            'corr_mean_f1_vs_var': corr,
            'reject_homoscedasticity': bool(lev_p < 0.05),
            'min_var': float(plan_vars.min()), 'max_var': float(plan_vars.max()),
...
def plot(summary: dict):
    ...
    if len(mean_f1) >= 2 and mean_f1.std() > 0:
        ...
        rp = np.corrcoef(mean_f1, var_f3)[0, 1]
        ax.plot(xx, np.polyval(z, xx), 'b--', linewidth=1.2, alpha=0.7,
                label=f"linear fit (r={rp:.2f})")
    ...
    ax2.set_title(r'Noise distribution varies across plans (ingolstadt21)')
    ...
    plt.tight_layout()
    for ext in ['png', 'pdf']:
        fp = os.path.join(FIG_DIR, f'figH_hetero_ingolstadt21.{ext}')
