#!/usr/bin/env python3
"""Submit the V23 factorized assignment/expert posterior gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v22_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v22_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _factorized(scale):
    value = dict(base._geometry(
        scale=float(scale),
        score="geometry_conditional",
        evidence_temperature=2.0,
    ))
    value["misspecification"] = "none"
    return value


VARIANTS = {
    "v15_tanh_control": dict(base.VARIANTS["v15_tanh_control"]),
    "v22_joint_geometry_loo": dict(
        base.VARIANTS["geometry_loo_s025_t20"]),
    "factorized_geometry_s005_t20": _factorized(0.05),
    "factorized_geometry_s025_t20": _factorized(0.25),
    "factorized_geometry_s100_t20": _factorized(1.0),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v23"
    args.gate_label = "Mean V23"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v23_offline_s5_"
