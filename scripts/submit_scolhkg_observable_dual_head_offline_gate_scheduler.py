#!/usr/bin/env python3
"""Submit the shared-observable mean/variance dual-head offline gate."""
...
VARIANTS = {
    "legacy_policy_control": {
        "mean_mode": "latent",
        "mean_input": "policy_profile",
        "mean_target": "constraint_mean",
        "variance_input": "legacy_policy_proxy",
    },
    "observable_mean_only": {
        "mean_mode": "boundary_aligned",
        "mean_input": "observable_state_exposure",
        "mean_target": "chance_margin",
        "variance_input": "legacy_policy_proxy",
    },
    "observable_variance_only": {
        ...
        "mean_input": "policy_profile",
        "mean_target": "constraint_mean",
        "variance_input": "observable_state_exposure",
    },
...
def _flags(profile, args):
    latent_dim = (
        int(args.rank)
        if profile["mean_input"] == "observable_state_exposure"
        else 2
    )
    return [
        ...
        "--observable-mean-mode", profile["mean_mode"],
        "--observable-mean-input-mode", profile["mean_input"],
        "--observable-mean-latent-dim", str(latent_dim),
        "--observable-mean-training-target", profile["mean_target"],
        "--observable-variance-input-mode", profile["variance_input"],
...
def build_specs(args):
    ...
    if not variants or any(variant not in VARIANTS for variant in variants):
        raise ValueError("unknown observable dual-head variant")
    if int(args.rank) not in {2, 4, 8}:
        raise ValueError("rank must be selected from {2,4,8}")
    if int(args.N) != int(args.n0):
        raise ValueError("offline dual-head gate requires N == n0")
        ...
        for heldout, shock_scale in SCENARIOS:
            ...
            for offset in range(int(args.n_seeds)):
                ...
                command = [
                    ...
                    "--seed", str(seed),
                    "--experiment-variant",
                    f"observable_dual_head_offline_r{args.rank}/{cell}",
                ...
                specs.append({
                    "description": (
                        f"Observable dual-head offline {variant} {heldout} "
                        f"shock={shock_scale:g} seed={seed}"
                    ),
                    "cmd": f"{shlex.join(command)} && echo DONE",
                    "cwd": str(local_project),
                    "signature": (
                        f"KG_op/observable_dual_head_offline/{args.run_id}/"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_observable_dual_head_offline_s5_"
