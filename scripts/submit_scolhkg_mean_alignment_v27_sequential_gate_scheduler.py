#!/usr/bin/env python3
"""Submit the V27 single exchangeable empirical-Bayes hyperlaw gate."""
...
BASE_PATH = (
    ROOT
    / "scripts/submit_scolhkg_mean_alignment_v26_sequential_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v26_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _aggregate(misspecification="none"):
    value = dict(base.VARIANTS["exchangeable_none"])
    value.update({
        "adaptation": "sequential_aggregate_hyperlaw",
...
VARIANTS = {
    ...
    "v26_sequential_mixture": dict(base.VARIANTS["exchangeable_none"]),
    "exchangeable_aggregate_none": _aggregate(),
    "exchangeable_aggregate_scale": _aggregate("predictive_scale"),
    "exchangeable_aggregate_directional": _aggregate(
...
def build_specs(args):
    ...
    args.stage_family = "mean_v27"
    args.gate_label = "Mean V27"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v27_sequential_s5_"
