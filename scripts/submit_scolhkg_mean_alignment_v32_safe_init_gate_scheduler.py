#!/usr/bin/env python3
"""Submit the V32 canonical-certificate incumbent-initialization gate."""
...
BASE_PATH = (
    ROOT
    / "scripts/submit_scolhkg_mean_alignment_v31_sandwich_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v31_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(misspecification, initialization):
    value = dict(base.VARIANTS["v29_direct_control"])
    value.update({
...
VARIANTS = {
    "v29_direct_control": _profile("none", "risk"),
    "v31_hc3_risk": _profile("predictive_sandwich_hc3", "risk"),
    "v32_hc3_cert": _profile(
        "predictive_sandwich_hc3", "certificate_lexicographic"),
    "v32_hc3_task_cert": _profile(
        "predictive_sandwich_hc3_task", "certificate_lexicographic"),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v32"
    args.gate_label = "Mean V32"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v32_safe_init_s5_"
    ...
    parser.add_argument(
        ...
        choices=("risk", "certificate_lexicographic"),
        default="risk",
    )
