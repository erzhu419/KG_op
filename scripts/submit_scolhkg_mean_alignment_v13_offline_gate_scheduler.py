#!/usr/bin/env python3
"""Submit the V13 source-support mean-coordinate gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v12_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v12_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(*, transform="identity", aggregate=False):
    return {
...
VARIANTS = {
    ...
    "v11_aggregate_control": dict(base.VARIANTS["v11_aggregate_control"]),
    "bounded_tanh_control": _profile(transform="source_tanh"),
    "support_clip_mixture": _profile(transform="source_support_clip"),
    "support_residual_mixture": _profile(
        transform="source_support_residual"),
    "support_clip_aggregate": _profile(
        transform="source_support_clip", aggregate=True),
    "support_residual_aggregate": _profile(
        transform="source_support_residual", aggregate=True),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v13"
    args.gate_label = "Mean V13"
    args.sequential = bool(int(args.N) > int(args.n0))
    return base.base.base.base.base.base.build_specs(args)


def main():
    parser = argparse.ArgumentParser()
    defaults = base.base.base.base.base.base
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v13_offline_s5_"
