#!/usr/bin/env python3
"""Submit the V15 independent mean/HVD task-posterior gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v14_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v14_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _isolated_profile(*, transform="identity", misspecification="none"):
    return {
        "descriptor": "role_adaptive_ordered",
        "adaptation": "sequential_evidence_mixture",
        "misspecification": str(misspecification),
        "role_epistemic": "none",
        "hvd_task_weight": "independent",
        "hvd_target_evidence": "replication_only",
        "hvd_singleton_evidence": "source_prior",
        "variance_task_posterior": "replication_only",
        "latent_transform": str(transform),
        "null_geometry": "isotropic",
    }


VARIANTS = {
    "v4_shared_control": dict(base.VARIANTS["v4_mixture_control"]),
    "v8_shared_control": dict(base.VARIANTS["v8_mixture_control"]),
    "bounded_shared_control": dict(base.VARIANTS["bounded_tanh_control"]),
    "isolated_linear": _isolated_profile(),
    "isolated_linear_contrast": _isolated_profile(
        misspecification="source_contrast"),
    "isolated_tanh": _isolated_profile(transform="source_tanh"),
    "isolated_tanh_contrast": _isolated_profile(
        transform="source_tanh", misspecification="source_contrast"),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v15"
    args.gate_label = "Mean V15"
    args.sequential = bool(int(args.N) > int(args.n0))
    return base.base.base.base.base.base.base.base.build_specs(args)


def main():
    parser = argparse.ArgumentParser()
    defaults = base.base.base.base.base.base.base.base
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v15_offline_s5_"
