#!/usr/bin/env python3
"""Materialize one immutable tracked-code snapshot for paper experiments."""
...
TRACKED_ROOTS = ("SC-OLH-KG", "proof", "scripts")
...
def _validate_existing(path, expected):
    ...
    if not marker.is_file():
        raise RuntimeError(f"snapshot directory has no marker: {path}")
    observed = json.loads(marker.read_text(encoding="utf-8"))
    if observed != expected:
        raise RuntimeError(f"snapshot contract mismatch: {path}")
    return observed
...
def materialize(repository_root, output_root, *, commit="HEAD"):
    ...
    try:
        ...
        with tarfile.open(archive, "r") as handle:
            # The archive is generated from this repository by Git, not
...
def main():
    ...
    parser.add_argument(
        "--output-root",
        default="/home/erzhu419/mine_code/KG_op_code_snapshots",
    )
