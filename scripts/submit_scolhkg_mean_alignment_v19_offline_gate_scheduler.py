#!/usr/bin/env python3
"""Submit the V19 Bayesian residual-rank mean-coordinate gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v18_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v18_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _rank_mixture(prior):
    value = base._profile(rank=2, prior_scale=0.25)
    value.update({
        "residual_rank_posterior": True,
        "residual_rank_prior": str(prior),
        "residual_rank_inactive_variance": 1e-12,
    })
    return value


VARIANTS = {
    "v15_tanh_control": base._profile(),
    "v18_rank2_fixed": base._profile(rank=2, prior_scale=0.25),
    "rank_mixture_complexity": _rank_mixture("0.70,0.20,0.10"),
    "rank_mixture_mild": _rank_mixture("0.50,0.30,0.20"),
    "rank_mixture_uniform": _rank_mixture("1.0,1.0,1.0"),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v19"
    args.gate_label = "Mean V19"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v19_offline_s5_"
