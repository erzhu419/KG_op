#!/usr/bin/env python3
"""Submit the sequential gate for a rank promoted by the offline gate."""
...
def _flags(variant, args):
    ...
    rank = 2 if variant == "latent_control" else int(args.rank)
    return [
        ...
        "--observable-mean-latent-dim", str(rank),
...
def build_specs(args):
    ...
    if not variants or any(variant not in VARIANTS for variant in variants):
        raise ValueError("unknown exposure-coordinate sequential variant")
    if int(args.rank) not in {2, 4, 8}:
        raise ValueError("rank must be promoted from {2,4,8}")
    if int(args.N) <= int(args.n0):
        raise ValueError("sequential gate requires N > n0")
        ...
        for heldout, shock_scale in SCENARIOS:
            ...
            for offset in range(int(args.n_seeds)):
                ...
                command = [
                    ...
                    "--seed", str(seed),
                    "--experiment-variant",
                    f"exposure_coordinate_sequential_r{args.rank}/{cell}",
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_exposure_coordinate_sequential_s5_"
