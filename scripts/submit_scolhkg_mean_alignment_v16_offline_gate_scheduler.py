#!/usr/bin/env python3
"""Submit the V16 partial-role-transport mean-calibration gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v15_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v15_submit", BASE_PATH)
...
def _profile(
    ...
    *,
    ...
    value = base._isolated_profile(
        transform="source_tanh",
        misspecification=misspecification,
    )
    value.update({
        "descriptor": str(descriptor),
        "role_epistemic": str(role_epistemic),
    })
    return value


VARIANTS = {
    "v15_tanh_control": _profile("role_adaptive_ordered"),
    "adaptive_uncertainty": _profile(
        "role_adaptive_ordered",
        role_epistemic="matching_uncertainty",
    ),
    "hard_role": _profile("role_aligned"),
    "hard_role_uncertainty": _profile(
        "role_aligned",
        role_epistemic="matching_uncertainty",
    ),
    "transport": _profile("role_transport"),
    "transport_uncertainty": _profile(
        "role_transport",
        role_epistemic="matching_uncertainty",
    ),
    "transport_uncertainty_contrast": _profile(
        "role_transport",
        role_epistemic="matching_uncertainty",
...
def build_specs(args):
    ...
    args.stage_family = "mean_v16"
    args.gate_label = "Mean V16"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v16_offline_s5_"
