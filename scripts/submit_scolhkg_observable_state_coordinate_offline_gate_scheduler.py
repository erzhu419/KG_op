#!/usr/bin/env python3
"""Submit V2's oracle-free observable state/trajectory coordinate gate."""
...
VARIANTS = {
    ...
    "learned_exposure_v1_phi_r4": {
        ...
        "rank": 4,
        "track": "oracle_free_v1_control",
    },
    **{
        f"observable_state_phi_r{rank}": {
            "mode": "boundary_aligned",
            "input": "observable_state_exposure",
            "target": "chance_margin",
            "rank": rank,
            "track": "oracle_free_v2_challenger",
        }
        for rank in (2, 4, 8)
    },
    **{
...
def build_specs(args):
    ...
    if not variants or any(variant not in VARIANTS for variant in variants):
        raise ValueError("unknown observable-state gate variant")
        ...
        for heldout, shock_scale in SCENARIOS:
            ...
            for offset in range(int(args.n_seeds)):
                ...
                command = [
                    ...
                    "--seed", str(seed),
                    "--experiment-variant",
                    f"observable_state_coordinate_offline/{cell}",
                ...
                specs.append({
                    "description": (
                        f"Observable state offline {profile['track']} {variant} "
                        f"{heldout} shock={shock_scale:g} seed={seed}"
                    ),
                    "cmd": f"{shlex.join(command)} && echo DONE",
                    "cwd": str(local_project),
                    "signature": (
                        f"KG_op/observable_state_coordinate_offline/"
                        f"{args.run_id}/{cell}/{heldout}/seed{seed}"
                    ),
                    "project": "KG-SYNTH",
                    "vram": 0,
                    "cpu": int(args.cpu),
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_observable_state_coordinate_offline_s5_"
