#!/usr/bin/env python3
"""Submit the V26 exchangeable target-linear mean-coordinate gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v25_sequential_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v25_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _exchangeable(*, misspecification="none", prior_df=4.0):
    value = dict(base.VARIANTS["v15_tanh_control"])
    value.update({
        "descriptor": "exchangeable_equivariant",
        ...
        "role_assignment_posterior": False,
        "role_assignment_prior": "uniform",
        "role_epistemic": "none",
        "latent_transform": "identity",
        "null_geometry": "isotropic",
        "structure_score": "marginal_likelihood",
    })
    return value
...
VARIANTS = {
    ...
    "v23_factorized_none": dict(base.VARIANTS["v23_factorized_none"]),
    "v25_boundary_s400": dict(base.VARIANTS["boundary_geometry_s400"]),
    "exchangeable_none": _exchangeable(),
    "exchangeable_hierarchical_df4": _exchangeable(
        misspecification="hierarchical_predictive_scale", prior_df=4.0),
    "exchangeable_hierarchical_df16": _exchangeable(
        misspecification="hierarchical_predictive_scale", prior_df=16.0),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v26"
    args.gate_label = "Mean V26"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v26_sequential_s5_"
