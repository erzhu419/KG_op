#!/usr/bin/env python3
"""Submit the V17 intervention-response mean-coordinate gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v16_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v16_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(descriptor, *, hierarchical=False):
...
VARIANTS = {
    ...
    "ordered_hierarchical": _profile(
        "role_adaptive_ordered", hierarchical=True),
    "intervention_transport": _profile("role_intervention_transport"),
    "intervention_hierarchical": _profile(
        "role_intervention_transport", hierarchical=True),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v17"
    args.gate_label = "Mean V17"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v17_offline_s5_"
