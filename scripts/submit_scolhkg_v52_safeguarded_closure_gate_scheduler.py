#!/usr/bin/env python3
"""Submit the paired V52 safeguarded statistical-closure sentinel gate."""
...
CONTROL = "v51_control"
CHALLENGERS = ("action_superset", "guarded_rollout", "joint")
...
def variant_profiles(args):
    common = {
        **closure.PROFILE,
        ...
        "policy_improvement_mc_error_bound": float(args.action_eta),
        "policy_improvement_rollout_depth": int(args.rollout_depth),
        "policy_improvement_rollout_max_arms": int(args.rollout_max_arms),
        "policy_improvement_rollout_mc_samples": int(args.rollout_mc_samples),
        "policy_improvement_rollout_mc_error_bound": float(args.rollout_eta),
    }
    return {
        CONTROL: {
            **common,
...
def build_specs(args):
    ...
    if unknown:
        raise ValueError(f"unknown V52 variants: {unknown}")
    ...
    args.stage_family = "v52_safeguarded_closure"
    args.gate_label = "V52 safeguarded statistical closure"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_v52_safeguarded_closure_s5_"
    ...
    parser.add_argument("--action-eta", type=float, default=0.0)
    parser.add_argument("--rollout-depth", type=int, default=2)
    parser.add_argument("--rollout-max-arms", type=int, default=4)
    parser.add_argument("--rollout-mc-samples", type=int, default=2)
    parser.add_argument("--rollout-eta", type=float, default=0.0)
