#!/usr/bin/env python3
"""Submit the V14 fully separated singleton-HVD gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v13_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v13_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(*, transform="identity", aggregate=False, isolated=False):
    return {
...
VARIANTS = {
    ...
    "bounded_tanh_control": dict(base.VARIANTS["bounded_tanh_control"]),
    "isolated_v8_mixture": _profile(isolated=True),
    "isolated_tanh_mixture": _profile(
        transform="source_tanh", isolated=True),
    "isolated_tanh_aggregate": _profile(
        transform="source_tanh", aggregate=True, isolated=True),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v14"
    args.gate_label = "Mean V14"
    args.sequential = bool(int(args.N) > int(args.n0))
    return base.base.base.base.base.base.base.build_specs(args)


def main():
    parser = argparse.ArgumentParser()
    defaults = base.base.base.base.base.base.base
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v14_offline_s5_"
