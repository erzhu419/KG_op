#!/usr/bin/env python3
"""Submit the preregistered observable-exposure sufficiency gate.

Each scheduler task freezes the same source archive and target initial design,
fits one constraint-mean coordinate, then audits one common post-run policy
pool. No sequential target query is made because ``N == n0`` by default.
"""
...
VARIANTS = {
    "latent_control": {
        ...
        "target": "constraint_mean",
        "rank": 2,
        "track": "oracle_free_control",
    },
    "profile_phi_r2": {
        "mode": "boundary_aligned",
        "input": "policy_profile",
        "target": "chance_margin",
        "rank": 2,
        "track": "oracle_free_control",
    },
    **{
        f"learned_exposure_phi_r{rank}": {
            "mode": "boundary_aligned",
            "input": "source_learned_exposure",
            "target": "chance_margin",
            "rank": rank,
            "track": "oracle_free_challenger",
        }
        for rank in (2, 4, 8)
    },
    **{
        f"provider_exposure_phi_r{rank}": {
            "mode": "boundary_aligned",
            "input": "provider_exposure",
            "target": "chance_margin",
            "rank": rank,
            "track": "structure_aware_upper_bound",
        }
        for rank in (2, 4, 8)
    },
}
...
def _flags(variant, args):
    ...
    return [
        ...
        "--observable-mean-mode", profile["mode"],
        "--observable-mean-input-mode", profile["input"],
        "--observable-mean-latent-dim", str(profile["rank"]),
        "--observable-mean-training-target", profile["target"],
...
def build_specs(args):
    ...
    if not variants or any(variant not in VARIANTS for variant in variants):
        raise ValueError("unknown exposure-coordinate gate variant")
    if int(args.N) != int(args.n0):
        raise ValueError("offline sufficiency gate requires N == n0")
        ...
        for heldout, shock_scale in SCENARIOS:
            ...
            for offset in range(int(args.n_seeds)):
                ...
                command = [
                    ...
                    "--seed", str(seed),
                    "--experiment-variant",
                    f"exposure_coordinate_offline/{cell}",
                ...
                specs.append({
                    "description": (
                        f"Exposure offline {profile['track']} {variant} "
                        f"{heldout} shock={shock_scale:g} seed={seed}"
                    ),
                    "cmd": f"{shlex.join(command)} && echo DONE",
                    "cwd": str(local_project),
                    "signature": (
                        f"KG_op/exposure_coordinate_offline/{args.run_id}/"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_exposure_coordinate_offline_s5_"
