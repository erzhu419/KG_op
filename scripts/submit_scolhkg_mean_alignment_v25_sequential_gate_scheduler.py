#!/usr/bin/env python3
"""Submit the V25 charged-pilot boundary-role posterior gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v24_sequential_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v24_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _boundary(scale, *, contrast=False):
    value = dict(base.base._factorized(float(scale)))
    value.update({
        "role_assignment_prior": "source_geometry_boundary",
        "misspecification": "source_contrast" if contrast else "none",
    })
    return value
...
VARIANTS = {
    ...
    "boundary_geometry_s100": _boundary(1.0),
    "boundary_geometry_s400": _boundary(4.0),
    "boundary_geometry_s100_contrast": _boundary(1.0, contrast=True),
    "boundary_geometry_s400_contrast": _boundary(4.0, contrast=True),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v25"
    args.gate_label = "Mean V25"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v25_sequential_s5_"
