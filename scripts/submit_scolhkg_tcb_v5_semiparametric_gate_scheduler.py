#!/usr/bin/env python3
"""Submit the strict TCB-V5 orthogonal semiparametric gate."""
...
DEFAULT_DEPLOY = (
    v3.shared.DEFAULT_DEPLOY.parent / "KG_op_scheduler_deploy_tcbv5")
    ...
    return [
        {
            **common,
            ...
            "rank": 1,
            "semiparametric_residual_features": 1,
            "semiparametric_residual_ridge": 1.0,
            "semiparametric_lengthscale_multiplier": 0.5,
        },
        {
            **common,
            "descriptor_mode": "raw",
            ...
            "rank": 1,
            "semiparametric_residual_features": 1,
            "semiparametric_residual_ridge": 10.0,
            "semiparametric_lengthscale_multiplier": 1.0,
        },
        {
            **common,
            "descriptor_mode": "raw",
            "geometry": "linear_monotone",
            "rank": 1,
            "semiparametric_residual_features": 2,
            "semiparametric_residual_ridge": 10.0,
            "semiparametric_lengthscale_multiplier": 0.5,
        },
        {
        ...
        },
        {
            **common,
            ...
            "rank": 2,
            "semiparametric_residual_features": 2,
            "semiparametric_residual_ridge": 100.0,
            "semiparametric_lengthscale_multiplier": 1.0,
        },
        {
            **common,
...
def command_for(args, config, heldout, seed, result_file):
    ...
    command = command.replace(
        ...
        "performance/benchmark_tcb_v5_semiparametric_gate.py",
        1,
    )
    command += " " + shlex.join([
        ...
        "--semiparametric-residual-features",
        str(int(config["semiparametric_residual_features"])),
        "--semiparametric-residual-ridge",
        str(float(config["semiparametric_residual_ridge"])),
        "--semiparametric-lengthscale-multiplier",
        str(float(config["semiparametric_lengthscale_multiplier"])),
    ])
    return command
...
def build_specs(args):
    return v3.shared.build_specs(
        ...
        model_tag="tcb_v5_semiparametric",
    )


def build_parser():
    parser = v3.build_parser()
    parser.description = "TCB-V5 orthogonal semiparametric boundary gate"
    parser.set_defaults(
        deploy=DEFAULT_DEPLOY,
        run_id=f"tcb_v5_semiparametric_{time.strftime('%Y%m%d_%H%M%S')}",
    )
    return parser
...
def main():
    ...
    if not args.dry_run and not args.skip_prepare_deploy:
        ...
        required = (
            target / "performance/benchmark_tcb_v5_semiparametric_gate.py")
        if not required.is_file():
            raise RuntimeError(f"prepared V5 deploy is missing {required}")
    ...
    manifest = {
        ...
        "offline_only": True,
        "model": "tcb_v5_orthogonal_semiparametric_boundary",
