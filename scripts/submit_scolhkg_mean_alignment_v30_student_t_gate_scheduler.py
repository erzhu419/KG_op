#!/usr/bin/env python3
"""Submit the V30 posterior-only Student-t mean-calibration gate."""
...
BASE_PATH = (
    ROOT
    / "scripts/submit_scolhkg_mean_alignment_v29_calibration_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v29_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(misspecification):
    value = dict(base.VARIANTS["v29_direct_dominance"])
    value["misspecification"] = str(misspecification)
    value["misspecification_delta"] = 0.05
    return value


VARIANTS = {
    "v29_direct_control": _profile("none"),
    "v29_scale_control": _profile("predictive_scale"),
    "v30_target_upper": _profile("predictive_scale_upper_target"),
    "v30_task_upper": _profile("predictive_scale_upper"),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v30"
    args.gate_label = "Mean V30"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v30_student_t_s5_"
