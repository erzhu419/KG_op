#!/usr/bin/env python3
"""Submit the V36 confidence-calibrated evaluate-or-replicate gate."""
...
SPEC = importlib.util.spec_from_file_location("mean_alignment_v35_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _profile(*, backend, adaptive, replicate_count):
    value = dict(base.VARIANTS["v35_confidence"])
    value.update({
        "decision_backend": str(backend),
        "adaptive_replication_voi": bool(adaptive),
        "replication_candidate_count": int(replicate_count),
...
VARIANTS = {
    "v35_sobol_new": _profile(
        backend="sobol_new", adaptive=False, replicate_count=0),
    "v36_joint_new_only": _profile(
        backend="sobol_joint_voi", adaptive=False, replicate_count=0),
    "v36_joint_rep4": _profile(
        backend="sobol_joint_voi", adaptive=True, replicate_count=4),
    "v36_joint_rep8": _profile(
        backend="sobol_joint_voi", adaptive=True, replicate_count=8),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v36"
    args.gate_label = "Mean V36"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v36_replication_s5_"
