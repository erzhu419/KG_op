#!/usr/bin/env python3
"""Submit the V22 source-geometry assignment-prior gate."""
...
BASE_PATH = (
    ROOT / "scripts/submit_scolhkg_mean_alignment_v21_offline_gate_scheduler.py"
)
SPEC = importlib.util.spec_from_file_location("mean_alignment_v21_submit", BASE_PATH)
base = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(base)


def _geometry(*, scale, score, evidence_temperature):
    value = dict(base.base._role_assignment())
    value.update({
        "role_assignment_prior": "source_geometry",
        "role_assignment_prior_temperature_scale": float(scale),
        "structure_score": str(score),
...
VARIANTS = {
    ...
    "v21_uniform_loo_t20": dict(base.VARIANTS["assignment_loo_t20"]),
    "geometry_marginal_s025": _geometry(
        scale=0.25, score="marginal_likelihood", evidence_temperature=1.0),
    "geometry_loo_s025_t20": _geometry(
        scale=0.25, score="loo_predictive", evidence_temperature=2.0),
    "geometry_loo_s100_t20": _geometry(
        scale=1.0, score="loo_predictive", evidence_temperature=2.0),
}
...
def build_specs(args):
    ...
    args.stage_family = "mean_v22"
    args.gate_label = "Mean V22"
...
def main():
    ...
    parser.add_argument("--run-id", default=(
        "scolh_mean_alignment_v22_offline_s5_"
